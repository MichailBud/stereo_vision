...
### Получение позиции робота из Gazebo

После запуска мира с плагином `libgazebo_ros_state.so` положение робота публикуется в топик `/gazebo/model_states`.

**robot_vac** — это спавн твоей модели robot_cameron (переименование в launch файле).

```bash
# Получить позицию robot_vac (индекс 21 в списке моделей)
ros2 topic echo /gazebo/model_states --csv 2>/dev/null | head -1 | awk -F',' '{print "x: " $22 "\ny: " $23 "\nz: " $24}'

# Получить ориентацию (кватернион)
ros2 topic echo /gazebo/model_states --csv 2>/dev/null | head -1 | awk -F',' '{print "qx: " $25 "\nqy: " $26 "\nqz: " $27 "\nqw: " $28}'
```

**Формат данных:** каждая модель занимает 7 значений — `x, y, z, qx, qy, qz, qw`

| Модель | Индекс | Поля |
|--------|--------|------|
| robot_vac | 21 | x: $22, y: $23, z: $24, qx: $25, qy: $26, qz: $27, qw: $28 |

**Пример вывода:**
```
x: 0.012913515916830976
y: -0.28628096390853264
z: -1.443330006688226e-05
qx: 1.3512966202209158e-05
qy: 7.469186018898998e-05
qz: -0.1402145818435234
qw: 0.990121136668121
```

**Альтернатива** — использовать одометрию:
```bash
